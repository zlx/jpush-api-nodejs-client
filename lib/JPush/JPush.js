    ...
    var _callback = function(err, res, body) {
        if (err) {
            if (err.code == 'ETIMEDOUT' && err.syscall != 'connect') {
                //response timeout
                return callback(new JError.APIConnectionError('Response timeout. Your request to the server may have already received, please check whether or not to push', true), {} JSON.parse(res.headers));
            } else if (err.code == 'ENOTFOUND') {
                //unknown host
                return callback(new JError.APIConnectionError('Known host : ' + url), {}, JSON.parse(res.headers));
            }
            //other connection error
            if (times < maxTryTimes) {
                return _request(url, body, headers, auth, method, times+1, maxTryTimes, callback);
            } else {
                return callback(new JError.APIConnectionError('Connect timeout. Please retry later.'), {}, JSON.parse(res.headers));
            }
        }
        if (res.statusCode == 200) {
            console.log("Push Success, response : " + body.toString())
            callback(null, JSON.parse(body), JSON.parse(res.headers));
        } else {
            console.log("Push Fail, HttpStatusCode: " + res.statusCode + " result: " + body.toString());
            callback(new JError.APIRequestError(res.statusCode, body), {}, JSON.parse(res.headers));
        }
    };
